...
export async function createBroker(
  ...
  try {
    ...
    return createSuccessResponse({ brokerId: result.id || `broker_${Date.now()}` })
  } catch (error) {
 ...
 * Get broker by ID
 */
export async function getBroker(brokerId: string): Promise<ApiResponse<Broker>> {
  try {
    const result = await apiClient.get<Broker>(`/brokers/${brokerId}`, () => getMockBroker(brokerId))
...
export async function getAllBrokers(): Promise<ApiResponse<Broker[]>> {
  try {
    const result = await apiClient.get<Broker[]>("/brokers", () => MOCK_BROKERS)
 ...
 */
export async function updateBroker(brokerId: string, input: BrokerUpdateInput): Promise<ApiResponse<void>> {
  try {
 ...
 */
export async function deleteBroker(brokerId: string): Promise<ApiResponse<void>> {
  try {
 ...
 */
export async function getBrokerClients(brokerId: string): Promise<ApiResponse<ClientSummary[]>> {
  try {
    const result = await apiClient.get<ClientSummary[]>(`/brokers/${brokerId}/clients`, () => getMockBrokerClients())
