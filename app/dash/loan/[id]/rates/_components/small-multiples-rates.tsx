...
function mockHistory(days: number, base = 6.75, amp = 0.08) {
  ...
  return Array.from({ length: days }).map((_, i) => {
    ...
    const jitter = Math.cos(Math.random()) * amp + Math.sin(i / 5) * amp + (Math.random() - 0.5) * 0.05
...
const PRODUCT_KEY_BY_TERM = (isJumbo: boolean, term: TermYears) => `${isJumbo ? "jumbo" : "conforming"}_fixed_${term}y`
...
function breakEvenMonths(closingCosts: number, currentPayment: number, newPayment: number): number | null {
...
function buildWindowedSeries(last90DaysHistory: RateHistory): RateHistory {
  ...
  last90DaysHistory.forEach((p) => map.set(format(new Date(p.date), "yyyy-MM-dd"), p.rate))
...
function useMortgageTermData(term: TermYears, jumbo: boolean) {
  ...
  const series = React.useMemo(() => (data ? buildWindowedSeries(data.history) : undefined), [data])
...
function TermCard({ term, color, jumbo, currentLoan, todayAgg, series }: TermCardProps) {
  const title = `${term}-Year ${jumbo ? "Jumbo" : "Fixed"} Rate Trend`
  const descr = "Past 90 days"

  const newPayment = pmt(currentLoan.outstandingPrincipalUsd, todayAgg.todayAvgRatePct, term * 12)
  ...
  const interestTotals = horizons.map((m) =>
    totalInterestOverMonths(currentLoan.outstandingPrincipalUsd, todayAgg.todayAvgRatePct, term * 12, m),
  )
  ...
  const closingCosts = todayAgg.avgClosingCostsUsd ?? currentLoan.assumedClosingCostsUsd ?? 0

  const beMonths = breakEvenMonths(closingCosts, currentPayment, newPayment)
  const beText =
    beMonths == null ? "No break-even (no savings)" : `${beMonths} mo (${format(monthsFromToday(beMonths), "PP")})`
  ...
  const finalDateDiffDays = Math.round(
    (startOfDay(finalNewDate).getTime() - startOfDay(finalCurDate).getTime()) / (1000 * 60 * 60 * 24),
  )
      ...
      <CardContent>
        <div className="h-36">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={series}>
              ...
              <XAxis dataKey="date" tickFormatter={(value) => format(new Date(value), "MMM d")} minTickGap={24} />
              <YAxis domain={["auto", "auto"]} tickFormatter={(v: number) => (v == null ? "" : `${v.toFixed(1)}%`)} />
        ...
        <div className="mt-4 grid grid-cols-2 gap-x-6 gap-y-2 text-sm">
          ...
          <div>
            ...
            <span className={monthlyDelta >= 0 ? "text-red-600" : "text-green-600"}>
              ({monthlyDelta >= 0 ? "+" : ""}
          ...
          <div>
            ...
            <span className={finalDateDiffDays >= 0 ? "text-red-600" : "text-green-600"}>
              ({finalDateDiffDays >= 0 ? "+" : ""}
              {Math.round(finalDateDiffDays / 30.4375)} months, {(finalDateDiffDays / 365.25).toFixed(1)} years)
            </span>
          </div>
...
/* =============================================================================
   ...
   ============================================================================= */
function SmallMultiplesRates({ currentLoan, countyFips }: MortgageSmallMultipleProps) {
  const { data: limitData, isLoading: limitLoading } = useSWR<ConformingLimit>(
    countyFips ? `/api/conforming-limit?countyFips=${countyFips}` : "/api/conforming-limit",
    fetcher,
  )
  ...
  if (errorAny) {
    return <div className="text-sm text-red-600">Error loading rates data. Please try again.</div>
  }
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="text-sm text-muted-foreground">
        All figures are estimates and should not be taken as gospel truth. To discover quotes specific to your
        situation, contact a lender or broker in the yellow pages.
      </div>
    </div>
  )
}
