...
export function LoanDetailsClient({ initialLoan }: LoanDetailsClientProps) {
  ...
  const [propertyPrice, setPropertyPrice] = useState<number>(loan.propertyPrice)
  ...
  const [startDate, setStartDate] = useState<string>(loan.startDate || toISODateLocal(new Date()))
  const [balance, setBalance] = useState<number>(loan.remainingBalance)
  const [creditScore, setCreditScore] = useState<string>(loan.creditScore || "700-749")
