...
export function PrefilledOnboarding({ data }: { data: PrefilledData }) {
  ...
  const [selectedAlerts, setSelectedAlerts] = useState<AlertConfig[]>(data.preSelectedAlerts || [])
  ...
  const [originalLoanAmount, setOriginalLoanAmount] = useState(data.originalLoanAmount)
  ...
  const handleFinalSubmit = async () => {
    ...
    try {
      ...
      router.push(`/dash/welcome?firstName=${encodeURIComponent(firstName)}&propertyId=${data.propertyId}`)
    } catch (error) {
        ...
        <div className="max-w-3xl mx-auto">
          <div className="mb-6 text-center">
            ...
            <p className="text-muted-foreground">
              Your broker has recommended these alerts. You can modify them or keep the recommendations.
            </p>
          </div>
