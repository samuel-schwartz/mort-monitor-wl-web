...
export async function getAdminInvoices() {
  try {
    const result = await apiClient.get<AdminInvoice[]>("/admin/invoices", () =>
      getMockInvoices(),
    );
...
export async function uploadInvoice(data: {
  ...
  try {
    ...
    return createSuccessResponse({
      id: result.data?.id || `inv_${Date.now()}`,
    });
  } catch (error) {
...
export async function getBrokerInvoices() {
  try {
    const result = await apiClient.get<AdminInvoice[]>("/broker/invoices", () =>
      getMockBrokerInvoices(),
    );
