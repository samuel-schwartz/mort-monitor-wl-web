...
): Promise<{ success: boolean; alertId?: string; subscriptionId?: string; error?: string }> {
  try {
    ...
    return { success: true, alertId: result.data?.id, subscriptionId: result.data?.subscriptionId }
  } catch (error) {
...
export async function getPropertyAlerts(
  ...
  try {
    ...
    if (!result.success) {
      return { success: false, error: result.error || "Failed to fetch alerts" }
    }
