  ...
  try {
    ...
    if (input.sendInvite && !invitationToken) {
      ...
      if (tokenResult.success && tokenResult.data) {
        ...
        console.log(
          "[v0] Generated invitation link: /onboard/invite/" + invitationToken,
        );
      }
    }
...
export async function getClient(clientId: string): Promise<ClientResponse> {
  try {
    const result = await apiClient.get<ClientResponse["client"]>(
      `/clients/${clientId}`,
    );
 ...
 * Get client by ID
 */
export async function reinviteClient(
  clientId: string,
): Promise<{ success: boolean; error?: string }> {
  try {
    const result = await apiClient.post(
      `/clients/${clientId}/reinvite`,
      {},
      () => {
        return { success: true };
      },
