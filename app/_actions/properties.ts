...
export async function createProperty(input: PropertyInput): Promise<ApiResponse<{ propertyId: string }>> {
  try {
    ...
    const result = await apiClient.post<{ id: string }>("/properties", { ...validatedInput, userId })
    ...
    revalidatePath("/dash")
    return createSuccessResponse({ propertyId: result.data?.id || `property_${Date.now()}` })
  } catch (error) {
...
export async function updateProperty(propertyId: string, input: PropertyUpdateInput): Promise<ApiResponse<void>> {
  try {
    ...
    const result = await apiClient.patch(`/properties/${propertyId}`, validatedInput)
